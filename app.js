...
async function autoSyncFromSheets() {
  ...
  try {
    ...
    console.log("autoSyncFromSheets:", changed ? "merged" : "no changes");
  } catch (e) {
  ...
  unitsList.forEach(unit => {
    ...
    card.innerHTML = `
      <div class="unit-card-info">
        ...
        <div class="unit-card-subtitle">${unit.subject} · ${unit.question_count}問</div>
      </div>
...
async function openUnit(unit) {
  currentUnit = unit;
  document.getElementById("unit-detail-title").textContent = unit.title;
...
async function renderQuiz() {
  ...
  // 縦書き: ページ進行は右→左なので、ボタンの有効/無効は逆
  // btn-prev-page（左ボタン=次ページ方向）: 最終ページで無効
  // btn-next-page（右ボタン=前ページ方向）: 最初のページで無効
  document.getElementById("btn-prev-page").disabled = currentPageIndex === activePages.length - 1;
  document.getElementById("btn-next-page").disabled = currentPageIndex === 0;
...
function setupEventListeners() {
  ...
  // 縦書き: 左ボタン=次ページ（ページインデックス増加）、右ボタン=前ページ（ページインデックス減少）
